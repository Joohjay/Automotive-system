...
(`APP_ID=automotive`, hostname `auto.bennyblax.co.tz`, port `4000`).
...
| App        | Port | Notes                        |
|------------|------|------------------------------|
| Automotive | 4000 | proxied by Nginx `/api/`     |
| Motorcycle | 4001 | future                       |
| …          | 4002 | future                       |

Ports are configurable via `API_PORT` in each app's config. **Never expose
internal ports (4000+) or PostgreSQL (5432) to the public internet.**
...
1. API health + database status (internal URL, e.g. `http://127.0.0.1:4000/api/health`)
...
**Never exposed publicly:** internal application ports (`4000`, `4001`, …) and
